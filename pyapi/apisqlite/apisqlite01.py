...
def movielookup(mykey, searchstring, type = ''):
    try:
        ## open URL to return 200 response
        resp = requests.get(f"{OMDBURL}apikey={mykey}&s={searchstring.split(0)}&type=searchstring.split(1)")
...
def harvestkey():
    with open("/Users/paullack/omdb.key") as apikeyfile:
